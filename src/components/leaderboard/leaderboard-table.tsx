...
export function LeaderboardTable() {
  ...
  const recordsQuery = useMemo(
    () => (firestore ? query(collection(firestore, "records")) : null),
    [firestore],
  );
  ...
  const [selectedDistrict, setSelectedDistrict] = useState<ScoreData | null>(
    null,
  );
  ...
  const leaderboardData: ScoreData[] = useMemo(() => {
    ...
    records.forEach((record) => {
      ...
      if (districtData) {
        ...
        if (!districtData.performance[record.category]) {
          districtData.performance[record.category] = {
            registered: 0,
            solved: 0,
          };
        }
        districtData.performance[record.category].registered +=
          record.casesRegistered || 0;
        districtData.performance[record.category].solved +=
          record.casesSolved || 0;
      }
    });
    ...
    const calculatedData = Array.from(districtScores.values()).map((d) => ({
      ...d,
      solveRate:
        d.totalCasesRegistered > 0
          ? (d.totalCasesSolved / d.totalCasesRegistered) * 100
          : 0,
  ...
  const getRowClass = (index: number) => {
    if (index === 0)
      return "bg-amber-100/50 dark:bg-amber-900/20 hover:bg-amber-100/70 dark:hover:bg-amber-900/30";
    if (index === 1)
      return "bg-slate-100/50 dark:bg-slate-800/20 hover:bg-slate-100/70 dark:hover:bg-slate-800/30";
    if (index === 2)
      return "bg-yellow-800/10 dark:bg-yellow-900/20 hover:bg-yellow-800/20 dark:hover:bg-yellow-900/30";
    return "";
  };
    ...
    <>
      <Card className="rounded-xl shadow-lg">
        <CardHeader>
          ...
          <CardDescription>
            {t("Based on total cases solved across all categories.")}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid gap-3 md:hidden">
            {isLoading
              ? Array.from({ length: 5 }).map((_, index) => (
                  <div key={index} className="surface-panel p-4 space-y-3">
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-semibold text-primary">
                        {item.overallScore.toLocaleString()}
                      </span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs text-muted-foreground">
                      <p>
                        {t("Registered")}:{" "}
                        {item.totalCasesRegistered.toLocaleString()}
                      </p>
                      <p>
                        {t("Solved")}: {item.totalCasesSolved.toLocaleString()}
                      </p>
                      <p>
                        {t("Solve Rate")}: {item.solveRate.toFixed(1)}%
                      </p>
                      <p>
                        {t("Overall Score")}:{" "}
                        {item.overallScore.toLocaleString()}
              ...
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-right">
                    {t("Total Cases Registered")}
                  </TableHead>
                  <TableHead className="text-right">
                    {t("Total Cases Solved")}
                  </TableHead>
                  <TableHead className="text-right">
                    {t("Solve Rate")}
                  </TableHead>
              ...
              <TableBody>
                {isLoading
                  ...
                  : leaderboardData.map((item, index) => (
                      <TableRow
                        key={item.id}
                        className={cn(getRowClass(index))}
                      >
                        <TableCell>
                          <div className="flex items-center gap-2">
                            {getRankIndicator(index)}
                        ...
                        <TableCell className="text-right">
                          {item.totalCasesSolved.toLocaleString()}
                        </TableCell>
                        <TableCell className="text-right">
                          {item.solveRate.toFixed(1)}%
                        </TableCell>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>
              {t("AI Improvement Suggestions for")} {selectedDistrict?.name}
            </DialogTitle>
            <DialogDescription>
              {t(
                "Based on the lowest performing categories, here are some actionable recommendations.",
              )}
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
