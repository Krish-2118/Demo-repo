    ...
    startProcessing(async () => {
      ...
      try {
        ...
        if (result && result.data && result.data.length > 0) {
          ...
          throw new Error(
            t("AI could not find any structured data in the text provided."),
          );
        }
      } catch (error) {
    ...
    startSaving(async () => {
      ...
      try {
        ...
        if (recordsAdded > 0) {
          ...
          throw new Error(
            t(
              "Upload failed. No valid records were found in the text. Please check the content.",
            ),
          );
        }
      } catch (error) {
      ...
      <Card>
        <CardContent className="pt-5 sm:pt-6">
          <div className="grid w-full gap-4">
            <Textarea
              placeholder={t(
                "Paste any unstructured text here. For example: 'On 2023-10-26, Ganjam district had 5 narcotics seizures, with 3 solved. Cuttack had 12 NBW executions registered and 10 solved.'",
              )}
              value={textInput}
