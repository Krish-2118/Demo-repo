...
export default function DashboardPage() {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return processedRecords.filter((r) => {
      ...
      const isDateInRange =
        filters.dateRange.from && r.date instanceof Date
  ...
  const prevMonthRecords = useMemo(() => {
    if (
      !previousMonthDateRange.from ||
      !previousMonthDateRange.to ||
      !processedRecords
    )
      return [];
    const selectedDistrictId =
    ...
    return processedRecords.filter(
      (r) =>
        ...
        isWithinInterval(r.date, {
          start: previousMonthDateRange.from!,
          end: previousMonthDateRange.to!,
        }),
    );
  ...
  const kpiData = useMemo((): PerformanceMetric[] => {
    ...
    return categories.map((category) => {
      ...
      const solveRate =
        currentRegistered > 0 ? (currentSolved / currentRegistered) * 100 : 0;
      const previousSolveRate =
        prevRegistered > 0 ? (prevSolved / prevRegistered) * 100 : 0;
  ...
  const districtPerformance = useMemo(() => {
    ...
    const performanceMap = new Map<
      number,
      { name: string; "Cases Registered": number; "Cases Solved": number }
    >();
  ...
  const trendData = useMemo(() => {
    ...
    const monthFormatter = new Intl.DateTimeFormat(langForFormatter, {
      month: "short",
      year: "numeric",
    });
    ...
    const relevantRecords = processedRecords.filter(
      (r) => r.date instanceof Date && r.date >= sixMonthsAgo,
    );
    ...
    relevantRecords.forEach((record) => {
      const dateObj =
        record.date instanceof Timestamp ? record.date.toDate() : record.date;
      ...
      if (
        monthData &&
        (filters.district === "all" ||
          record.districtId === selectedDistrictId) &&
  ...
  return (
    <div className="flex-1 space-y-5 sm:space-y-6 stagger-in">
      <Filters
